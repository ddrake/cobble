...
class CobblePage extends Object {
  ...
  function ParseHardThemePaths() {
    ...
    $themePaths = array_merge($themePaths, $this->ParseHardThemePath($this->stdpath(DIR_BASE . '/' . 'config' . '/' . 'site_theme_paths.php')));
  ...
  function AddFromRow($row) {
    ...
    } else if(!empty($ctID) && empty($ctID_ct) && !empty($ptDirectory)) {
    ...
    $db = Loader::db();
    $v = array($cID, $ctID, $cblPtID, $this->stdpath($filePath),
          $this->stdpath($wrapperPath), $isThemeFromPath, $isSinglePage, $cIsTemplate, $isWrapperMissing, $isFileMissing,
          $cFilename, $cHandle);
  ...
  function GetThemeForSinglePageWithThemeNotSet($cHandle) {
    ...
    } else if (file_exists(DIR_FILES_THEMES . '/' . 'default' . '/' . FILENAME_THEMES_VIEW)) {
      $theme = DIR_FILES_THEMES . "/" . 'default' . '/' . FILENAME_THEMES_VIEW;
    } else if (file_exists(DIR_FILES_THEMES . '/' . DIRNAME_THEMES_CORE . '/' . 'default' . '.php')) {
      $theme = DIR_FILES_THEMES . '/' . DIRNAME_THEMES_CORE . "/" . 'default' . '.php';
    } else if (file_exists(DIR_FILES_THEMES_CORE . "/" . 'default' . '/' . $cHandle)) {
      $theme = DIR_FILES_THEMES_CORE . "/" . 'default' . '/' . $cHandle;
    } else if (file_exists(DIR_FILES_THEMES_CORE . "/" . 'default' . '/' . FILENAME_THEMES_VIEW)) {
      $theme = DIR_FILES_THEMES_CORE . "/" . 'default' . '/' . FILENAME_THEMES_VIEW;
    } else if (file_exists(DIR_FILES_THEMES_CORE_ADMIN . "/" . 'default' . '.php')) {
  ...
  function GetRegularPageTypePath($ctHandle, $ptDirectory) {
    ...
    } else if (file_exists($ptDirectory . '/' . 'default.php' )) {
  ...
  function listSubdirectories ($dir, $singlePageOnly=false) {
    ...
    if (is_array($dirs)) {
      foreach($dirs as $subdir) {
        if (($singlePageOnly==true && basename($subdir) == 'single_pages') || ($singlePageOnly==false && !in_array($subdir,array('.','..')))) {
