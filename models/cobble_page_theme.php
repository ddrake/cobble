...
class CobblePageTheme extends Object {
  ...
  function setThemeLists() {
    $this->packageThemes = array_merge($this->listThemes_forPackageDir(DIR_PACKAGES), $this->listThemes_forPackageDir(DIR_PACKAGES_CORE));
    $this->nonPackageThemes = array_merge($this->listThemes_forNonPackageDir(DIR_FILES_THEMES), $this->listThemes_forNonPackageDir(DIR_FILES_THEMES_CORE));
  }
  ...
  function listSubdirectories ($dir, $themeOnly=false) {
    ...
    if (is_array($dirs)) {
      foreach($dirs as $subdir) {
        if (($themeOnly==true && basename($subdir) == 'themes') || ($themeOnly==false && basename($subdir) != 'core' && !in_array($subdir,array('.','..')))) {
