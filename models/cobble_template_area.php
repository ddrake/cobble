...
class CobbleTemplateArea extends Object
{
    ...
    function getIncludeInfo($ta, $i_start)
    {
        ...
        $i++;
        $val = $ta[$i];
        if (!is_array($val) || $val[0] != T_STRING || $val[1] != 'inc') {
            return null;
        }
        ...
        $i++;
        $val = $ta[$i];
        if (!is_array($val) || $val[0] != T_CONSTANT_ENCAPSED_STRING) {
            return null;
        }
    ...
    function getAreaInfo($ta, $i_start)
    {
        ...
        $i++;
        $val = $ta[$i];
        if (!is_array($val) || $val[0] != T_STRING || !in_array($val[1], array('Area', 'GlobalArea'))) {
            return null;
        }
