  ...
  function addFromPageTypes() { 
    ...
    $r = $db->query($q);
      ...
      if (!empty($ctp)) {
        ...
        $v = array(basename($ctp,'.php'), $ctp, $row['ctID'], $this->defaultWrapperPath, $row['pkgHandle'], $row['ctHandle']);
  ...
  function listSubdirectories ($dir, $pageTypeOnly=false) {
    ...
    if (is_array($dirs)) {
      foreach($dirs as $subdir) {
        if (($pageTypeOnly==true && basename($subdir) == 'page_types') || ($pageTypeOnly==false && !in_array($subdir,array('.','..')))) {
